...
char* func(char *c, int max_size, int main_size)
{
    ...
    {
        if(i<main_size && c[i]!='.' && c[i]!='-' && (c[i]<65 || c[i]>122) && !(c[i]>='0' && c[i]<='9'))
        {
            space_ase  = 1;
        }
...
void show()
{
    if(l==0) {
        for(int i=0; i<size; i++)
        {
            ...
            if(karo_space_ase==1 && allinfo[i].quoted == 0)
            {
                printf(" ");
            }
            if(allinfo[i].quoted == 1)
            {
...
void alphabetically_sort()
{
    ...
    for(int i=0; i<size ; i++)
    {
        for(int j=0; j<size - i -1; j++)
        {
            ...
            if(space_ase == 1)
            {
                allinfo[j+1].quoted = 1;
                karo_space_ase = 1;
            }
            
            //if(strcmp(allinfo[j].name,"?a")==0)

            //printf("hello:  %s\n", p);
             //if(strcmp(allinfo[j].name,"..")==0)
